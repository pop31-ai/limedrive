...
| бесконечный бег (с соперниками!) | endless-runner | 4 |
...
# РЕЖИМЫ ДВИЖКА v1.2 — надстройки над платформером (день 2)

Движок сам включает режим по свойствам героя/сущностей — отдельный type не нужен.

## 8) RPG-бой (hero.properties)
| Свойство | Смысл |
|----------|-------|
| attackDamage / attackRange / attackCooldown | удар Space/Enter по врагам в радиусе |
Ключи: pickup { pickupType:"key", keyId:"gold" }.
Двери: trigger { locked:true, requiredKey:"gold" } — открывается касанием с ключом.

## 9) Кислород и плавание (hero.properties)
| Свойство | Смысл |
|----------|-------|
| maxOxygen / oxygenDrain | запас O₂ и расход в секунду под водой |
| swimForce / diveSpeed | тяга вверх (стрелка Up) и вниз |
Поверхность (верх ~13% мира) пополняет O₂. Пикап { pickupType:"oxygen", value }.
O₂=0 → урон каждую секунду. HUD-шкала O₂ включается автоматически.

## 10) Свет и тьма (hero.properties)
| Свойство | Смысл |
|----------|-------|
| lanternRadius / lanternFlicker | радиус света героя (фликер — доля дрожания) |
Враги со свойствами visibleInLight:true или phaseInWalls:true невидимы
и безвредны вне радиуса фонаря. Тьма — авто-оверлей.

## 11) Стелс (enemy.properties + hero)
| Свойство | Где | Смысл |
|----------|-----|-------|
| visionRange / visionAngle | enemy | конус зрения (px / полный угол °) |
| sneakMultiplier | hero | скорость с Shift (и радиусы конусов ×0.55) |
В конусе → тревога: охрана преследует 2.5с. Терминалы:
trigger { hackTime:2, targetId:"door_1" } — стой рядом, цель удалится.

## 12) Капитал (settings или finish.properties)
| Свойство | Смысл |
|----------|-------|
| capitalGoal | финиш открывается только при score ≥ цели; HUD «КАПИТАЛ» |

## 13) Гравитационные поля (platform.properties)
| Свойство | Смысл |
|----------|-------|
| gravityField:"low"/"high"/"flip" + radius | множитель гравитации в радиусе |
flip: защёлкивается до приземления на обычный пол; можно ходить по потолку
(низ платформ), прыжок направлен вниз. Улетел выше y<-300 — смерть.

## 14) Соперники в раннере (enemy.properties)
| Свойство | Смысл |
|----------|-------|
| rival:true, rivalSpeed, rivalName | бегун-конкурент; HUD «POS n/m» |
Если соперников нет — движок сам ставит двух (0.96× и 1.04× скорости героя).
Касание соперника безвредно.

## Нейрогенератор черновиков
`node tools/neuro-game-nn.js generate <genre>` — MLP учится на всех играх
и выдаёт `_neuro-draft-<genre>.json` (проходит валидацию). Черновик —
как заготовка для промпта «вот JSON, улучши».

---

# TBS-ВАРИАЦИИ (settings.variant) — v1.3

Все наследуют шахматный формат (grid + piece). Включаются через settings.variant.

## Шашки (variant: "checkers" или movePattern:"checkers")
| Свойство | Смысл |
|----------|-------|
| pieceType:"checker" + movePattern:"checkers" | шашка: диагональ вперёд, прыжок через врага |
| props.isKing (авто) | дамка: все 4 диагонали |
Правила движка: бой ОБЯЗАТЕЛЕН (простые ходы блокируются), цепочки прыжков
продолжают ход, последняя линия = дамка. Победа: все фигуры врага мертвы.

## Ци-Шах (variant: "qi")
| Свойство (props фигуры) | Смысл |
|----------|-------|
| str | сила: урон попадания = 10 + str*4 + темп*5 |
| energy | каждая атака тратит 1 |
| tempo (авто) | парирование +1 (макс 2); конвертируется в урон при попадании |
Исход атаки: попадание / парирование / контратака (враг отходит и бьёт).
settings.qiForce = hit|parry|counter — фиксирует исход (для тестов).
Дуэль 1×1, арена 8×2..8×4, победа = нокаут врага.

## Элиминтали (variant: "elem")
| Свойство (props) | Смысл |
|----------|-------|
| elem: fire/water/air/earth + tier: 1..3 | стихийная фигура |
| movePattern по уровню | T1=adjacent, T2=l-shape, T3=any-direction (до 3) |
| pieceType:"core" + movePattern:"none" | ядро = король, health 1 |
Слияние: свои элементали равного уровня рядом → уровень+1 (разные стихии =
гибрид: Молния/Магма/Пар/Песок/Грязь/Туман). Цикл: огонь→воздух→земля→вода→огонь;
гибрид бьёт чистую, если содержит её стихию; ничья → старший уровень.
Проигрывающие захваты движок не предлагает. Победа = захват ядра.

## Зеркальный шах (variant: "mirror")
| Свойство | Смысл |
|----------|-------|
| health у фигур | HP: король 3, ферзь/ладья/слон/конь 2, пешка 1 |
Захват будит призрака жертвы на ЗЕРКАЛЬНОЙ клетке (строка 7−r, тот же столбец):
призрак бьёт атакующего на 1 HP, если (а) зеркало не закрыто своей фигурой и
(б) роль жертвы с зеркала достаёт. Победа = захват вражеского короля.

## Детерминизм
settings.seed — сид ПСЧРГ (mulberry32) на уровень: одинаковые броски при каждом запуске.
